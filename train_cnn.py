"""
Deep Learning Model: CNN on Regular Spectrograms
"""
...
def train_model(model, train_loader, val_loader, class_weight=None, epochs=30, lr=0.001):
    """
    Train the CNN model with early stopping and validation monitoring.
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print("CNN SPECTROGRAM MODEL TRAINING")
    print("="*60)
    ...
    # pos_weight = count of negative class / count of positive class
    ...
    print("\nCreating CNN Spectrogram model...")
    model = SpectrogramCNN().to(device)
    print(f"Model parameters: {sum(p.numel() for p in model.parameters()):,}")
    
    print("\nTraining CNN Spectrogram model with class weighting...")
    print("-" * 60)
    model = train_model(model, train_loader, val_loader, class_weight=class_weight)
    
    # Evaluate
    print("\n" + "="*60)
    print("EVALUATING CNN SPECTROGRAM MODEL")
    print("="*60)
    ...
    torch.save(model.state_dict(), os.path.join(models_dir, 'cnn_spectrogram.pth'))
    joblib.dump(label_encoder, os.path.join(models_dir, 'cnn_spec_label_encoder.pkl'))
    
    print(f"\nModel saved to {os.path.join(models_dir, 'cnn_spectrogram.pth')}")